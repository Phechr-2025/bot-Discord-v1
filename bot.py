...
import re  # <-- added for link normalization
...
def _clean_link(s: str) -> str:
    # remove quotes/spaces/<> accidentally pasted
...
def normalize_gdrive_for_download(url_or_id: str) -> str:
    ...
    Accepts:
      ...
      - bare id
    Returns canonical https://drive.google.com/uc?id=<id>
    """
    s = _clean_link(url_or_id)
    fid = _gdrive_file_id(s)
    if fid:
        return f"https://drive.google.com/uc?id={fid}"
    return s  # maybe already direct link
...
async def deliver(
    *,
    ...
    # method == file with timeout + fallback
    try:
        ...
        if size <= MAX_UPLOAD_BYTES:
            ...
            await target.send(
                content=(
                    ...
                    f"เกินลิมิตอัปโหลดของ Discord ครับ 🙏\nลิงก์ดาวน์โหลด: {clean_link}"
                )
            )
    except asyncio.TimeoutError:
        await target.send(
            content=(
                f"การเตรียมไฟล์ **{item_name}** ใช้เวลานาน ⏳\n"
                f"ขอส่งเป็นลิงก์ดาวน์โหลดแทนก่อนนะครับ:\n{clean_link}"
            )
        )
...
class ConfirmBuyView(View):
    ...
    @discord.ui.button(label="ส่งเป็นไฟล์ + DM", style=discord.ButtonStyle.success, custom_id='shop:file_dm')
    async def file_dm(self, interaction: Interaction, button: Button):
        await self._handle(interaction, "file", "dm")

    @discord.ui.button(label="ส่งเป็นไฟล์ + ในห้องนี้", style=discord.ButtonStyle.success, custom_id='shop:file_chan')
    async def file_chan(self, interaction: Interaction, button: Button):
        await self._handle(interaction, "file", "channel")

    @discord.ui.button(label="ส่งเป็นลิงก์ + DM", style=discord.ButtonStyle.primary, custom_id='shop:link_dm')
    async def link_dm(self, interaction: Interaction, button: Button):
        await self._handle(interaction, "link", "dm")

    @discord.ui.button(label="ส่งเป็นลิงก์ + ในห้องนี้", style=discord.ButtonStyle.primary, custom_id='shop:link_chan')
