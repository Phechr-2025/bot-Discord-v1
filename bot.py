...
def db_init():
    with db() as conn:
        ...
        # เพิ่ม guild_settings แยกตามกิลด์
        c.execute("""
...
# ---------- Logging helpers ----------
async def log_to_guild_channel(guild: Optional[discord.Guild], key: str, text: str):
    if guild is None:
        return
    cid_str = get_guild_setting(guild.id, key, "")
    if not cid_str:
        return
    try:
        cid = int(cid_str)
...
class ConfirmBuyView(View):
    ...
    async def _handle(self, interaction: Interaction, dest: Literal["dm", "channel"]):
        ...
        # เลือกช่องกลุ่มจาก guild setting ถ้า dest == channel
        channel = None
        if dest == "channel":
        ...
        # log purchase
        await log_to_guild_channel(
            interaction.guild,
            "log_purchase_channel_id",
...
# ---- Admin set channels ----
@bot.tree.command(name="admin_set_send_channel", description="(แอดมิน) กำหนดห้องสำหรับส่งวิดีโอในกลุ่ม")
@app_commands.describe(channel="เลือกห้อง")
async def admin_set_send_channel(interaction: Interaction, channel: discord.TextChannel):
    msg = require_admin(interaction)
    if msg:
        return await interaction.response.send_message(msg, ephemeral=True)
    if interaction.guild is None:
        return await interaction.response.send_message("ต้องใช้ในกิลด์เท่านั้น", ephemeral=True)
    set_guild_setting(interaction.guild.id, "send_channel_id", str(channel.id))
    await interaction.response.send_message(f"ตั้งค่าห้องส่งวิดีโอเป็น {channel.mention} แล้ว", ephemeral=True)

@bot.tree.command(name="admin_set_log_transfer", description="(แอดมิน) กำหนดห้องบันทึกประวัติการโอน")
@app_commands.describe(channel="เลือกห้อง")
async def admin_set_log_transfer(interaction: Interaction, channel: discord.TextChannel):
    msg = require_admin(interaction)
    if msg:
        return await interaction.response.send_message(msg, ephemeral=True)
    if interaction.guild is None:
        return await interaction.response.send_message("ต้องใช้ในกิลด์เท่านั้น", ephemeral=True)
    set_guild_setting(interaction.guild.id, "log_transfer_channel_id", str(channel.id))
    await interaction.response.send_message(f"ตั้งค่าห้องประวัติการโอนเป็น {channel.mention} แล้ว", ephemeral=True)

@bot.tree.command(name="admin_set_log_purchase", description="(แอดมิน) กำหนดห้องบันทึกประวัติการซื้อ")
@app_commands.describe(channel="เลือกห้อง")
async def admin_set_log_purchase(interaction: Interaction, channel: discord.TextChannel):
    msg = require_admin(interaction)
    if msg:
        return await interaction.response.send_message(msg, ephemeral=True)
    if interaction.guild is None:
        return await interaction.response.send_message("ต้องใช้ในกิลด์เท่านั้น", ephemeral=True)
    set_guild_setting(interaction.guild.id, "log_purchase_channel_id", str(channel.id))
    await interaction.response.send_message(f"ตั้งค่าห้องประวัติการซื้อเป็น {channel.mention} แล้ว", ephemeral=True)

@bot.tree.command(name="admin_set_log_topup", description="(แอดมิน) กำหนดห้องบันทึกประวัติการเติมเงิน")
@app_commands.describe(channel="เลือกห้อง")
async def admin_set_log_topup(interaction: Interaction, channel: discord.TextChannel):
    msg = require_admin(interaction)
    if msg:
        return await interaction.response.send_message(msg, ephemeral=True)
    if interaction.guild is None:
        return await interaction.response.send_message("ต้องใช้ในกิลด์เท่านั้น", ephemeral=True)
    set_guild_setting(interaction.guild.id, "log_topup_channel_id", str(channel.id))
    await interaction.response.send_message(f"ตั้งค่าห้องประวัติการเติมเงินเป็น {channel.mention} แล้ว", ephemeral=True)
