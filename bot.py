...
# ======================
#   HARD-CODED CHANNEL IDS (แก้ตัวเลขให้ตรงกับเซิร์ฟเวอร์ของคุณ)
# ======================
SEND_CHANNEL_ID = int(os.getenv("SEND_CHANNEL_ID", "0"))              # ห้องสำหรับส่งวิดีโอในกลุ่ม
LOG_TRANSFER_CHANNEL_ID = int(os.getenv("LOG_TRANSFER_CHANNEL_ID", "0"))  # ห้องบันทึกประวัติการโอน
LOG_PURCHASE_CHANNEL_ID = int(os.getenv("LOG_PURCHASE_CHANNEL_ID", "0"))  # ห้องบันทึกประวัติการซื้อ
LOG_TOPUP_CHANNEL_ID = int(os.getenv("LOG_TOPUP_CHANNEL_ID", "0"))        # ห้องบันทึกประวัติการเติมเงิน

# หากต้องการ "ระบุในโค้ดล้วนๆ" ไม่พึ่ง ENV ให้แก้ค่าคงที่ด้านล่างแทน (จะ override ค่า ENV ด้านบน)
# ตัวอย่าง:
# SEND_CHANNEL_ID = 123456789012345678
# LOG_TRANSFER_CHANNEL_ID = 234567890123456789
# LOG_PURCHASE_CHANNEL_ID = 345678901234567890
# LOG_TOPUP_CHANNEL_ID = 456789012345678901
...
INTENTS.members = True   # สำคัญสำหรับ user picker
...
async def deliver_file(
    *,
    ...
    """
    ส่งไฟล์ไปที่:
     - ถ้ามี SEND_CHANNEL_ID (>0 และเจอห้อง) => ส่งในห้องนั้น
     - ไม่งั้นส่งทาง DM
    """
    target = None
    if guild and SEND_CHANNEL_ID > 0:
        ch = guild.get_channel(SEND_CHANNEL_ID) or bot.get_channel(SEND_CHANNEL_ID)
        if ch is None:
            try:
                ch = await bot.fetch_channel(SEND_CHANNEL_ID)
            except Exception:
                ch = None
        if ch:
            target = ch
...
# ---------- Logging helpers ----------
async def log_by_fixed_ids(guild: Optional[discord.Guild], *, kind: str, text: str):
    if guild is None:
        return
    channel_id = 0
    if kind == "transfer":
        channel_id = LOG_TRANSFER_CHANNEL_ID
    elif kind == "purchase":
        channel_id = LOG_PURCHASE_CHANNEL_ID
    elif kind == "topup":
        channel_id = LOG_TOPUP_CHANNEL_ID
    if channel_id <= 0:
        return

    ch = guild.get_channel(channel_id) or bot.get_channel(channel_id)
    if ch is None:
        try:
...
class ConfirmBuyView(View):
    ...
    async def _handle(self, interaction: Interaction, dest: Literal["dm", "channel"]):
        ...
        # หากเลือกส่ง "ในห้อง" ให้บังคับใช้ห้องที่กำหนดด้วย SEND_CHANNEL_ID
        # ถ้าไม่ได้ตั้งค่า หรือหาไม่เจอ จะ fallback เป็น DM อัตโนมัติ
        ok, err = await deliver_file(
            user=interaction.user,
            item_name=item["name"],
            gdrive_url=item["gdrive_url"],
            filename=item["filename"] or "video.mp4",
            guild=interaction.guild if dest == "channel" else None,
        )
        ...
        if interaction.guild:
            await log_by_fixed_ids(
                interaction.guild,
                kind="purchase",
                text=f"🛒 **Purchase** by {interaction.user.mention} | Item: **{item['name']}** | Price: {fmt_thb(price)} | {now_utc_iso()}",
            )
        ...
        if ok:
            ...
            if interaction.guild:
                await log_by_fixed_ids(
                    interaction.guild,
                    kind="transfer",
                    text=f"🤝 **Transfer** {interaction.user.mention} → {to_display} | Amount: {amount_thb:.2f} บาท | {now_utc_iso()}",
                )
        else:
    ...
    if ok:
        ...
        if interaction.guild:
            await log_by_fixed_ids(
                interaction.guild,
                kind="transfer",
                text=f"🤝 **Transfer** {interaction.user.mention} → {user.mention} | Amount: {amount_thb:.2f} บาท | {now_utc_iso()}",
            )
    else:
        await interaction.response.send_message(f"โอนเงินไม่สำเร็จ: {msg}", ephemeral=True)
...
async def admin_add_balance(interaction: Interaction, user: discord.User, amount_thb: float):
    ...
    if interaction.guild:
        await log_by_fixed_ids(
            interaction.guild,
            kind="topup",
            text=f"➕ **Topup** {user.mention} by {interaction.user.mention} | Amount: {amount_thb:.2f} บาท | {now_utc_iso()}",
        )
...
async def admin_check_balance(interaction: Interaction, user: discord.Member):
    msg = require_admin(interaction)
    if msg:
        return await interaction.response.send_message(msg, ephemeral=True)
...
async def on_ready():
    ...
    # persistent view (เมนูไม่หมดอายุจนกว่าจะรีสตาร์ทบอท)
